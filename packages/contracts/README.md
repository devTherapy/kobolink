...
| GET | `links.collection` | merchant | `PageQuery` | `LinkListResponse` | Only the caller's links, newest first by `createdAt` |
...
| GET | `links.payments(code)` | merchant | `PageQuery` | `PaymentListResponse` | Newest first. **Includes failed payments** (`status: 'failed'`, `failureReason`, `moneyMoved: false`) so the merchant sees declines; `PaymentLink.paymentCount`/`totalPaidKobo` and `DashboardStats` count successes only. `payerEmail` is masked |
...
| POST | `checkout.verify` | none | `VerifyCheckoutRequest` + `Idempotency-Key` | `VerifyCheckoutResponse` | Re-resolves the link **inside the posting transaction**; if it is no longer `payable` (disabled, expired, or already paid by a racing initialize on a single-use link) the payment is recorded as `status: 'failed'` with a `failureReason` naming the state and `moneyMoved: false` — never a success. Otherwise on success posts the ledger entries in that transaction and answers `payment.moneyMoved: true`. A gateway decline is **200**, not an error: `payment.status: 'failed'`, `failureReason` set, `moneyMoved: false`. Simulated gateway: `payerEmail` starting `fail@` declines. Verifying the same `reference` again — under any key — returns the original payment; a reference is decided once |
