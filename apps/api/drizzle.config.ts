...
/**
 * `npx drizzle-kit generate` diffs `src/db/schema/index.ts` (which re-exports
 * every table from its sibling files) against `drizzle/` and writes a new
 * migration. `DbService`'s Testcontainers-backed integration tests apply the
 * same `drizzle/` folder with `drizzle-orm`'s migrator, so what `generate`
 * produces here is exactly what boots against a real container.
 *
